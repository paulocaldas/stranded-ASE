...
if [[ ! -f "${INPUT_BAM}.bai" && ! -f "${INPUT_BAM%.bam}.bai" ]]; then
  echo "Error: BAM index not found for '${INPUT_BAM}'."
  echo "Please run: samtools index ${INPUT_BAM}"
  exit 1
fi

if [[ ! -f "$VARIANTS_VCF" ]]; then
  echo "Error: Variants VCF file '${VARIANTS_VCF}' not found."
  exit 1
fi

if [[ ! -f "${VARIANTS_VCF}.tbi" ]]; then
  echo "Error: VCF index (.tbi) not found for '${VARIANTS_VCF}'."
  echo "Please run: bcftools index ${VARIANTS_VCF}"
  exit 1
fi
...
echo "  Variants VCF:       ${VARIANTS_VCF}"
